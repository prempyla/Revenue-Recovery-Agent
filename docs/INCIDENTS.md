...
## External cold review found three real gaps our own audits missed

**Stage:** After the 8-item architecture audit, the P1 fixes, and the kill -9/one-command-demo round — every internal check to that point (215→238 tests, the self-audit, the stress tests) had come from us reading our own code. This round is different in kind and worth naming as such, per instruction: a reviewer with no build history, reading the repo cold, found three things our own process — adversarial as it tried to be — never caught. Credited honestly: not found by our own testing, our own audit, or our own stress-testing. Found by someone else reading the code fresh, which is exactly the blind spot a team auditing its own work structurally can't fully cover.

**Finding 1 (the worst of the three): `decide()`'s `now` parameter was dead code.** `full_agent.decide(payment, customer, now, tracker, failure_log, outage_events, ...)` accepted `now` and never read it — not on any branch, not partially: confirmed by grepping every reference to `now` inside `decide()`, `_candidates_for()`, and `_veto_reason()` before changing a line, per instruction to verify before acting. `action_time` was always `payment.failed_at + candidate.offset`; the returned offset was always the raw candidate offset. Consequence: `llm.reply_handling.apply_reply_intent`'s PROMISE_TO_PAY path — a customer replies "I'll pay by the 10th," the system computes a `reeval_time` from that promise and calls `decide()` again — got back the byte-identical decision it would have gotten at the *original* failure time, regardless of what the customer actually promised. Not a crash, not a wrong-looking number — a feature that looked wired up (the parameter exists, is threaded through, is documented as making `decide()` "pure... `now`/tracker/failure_log/outage_events are all explicit arguments") and silently did nothing.

**Why it was dangerous:** the exact shape the "recurring pattern" section above names, one level up: not a false *reason* recorded in the audit log, but a false *claim* in the code's own docstring and in the README's purity argument — "explicit inputs" implied `now` was live, and it wasn't. A promise-to-pay re-evaluation is precisely the feature a payments reviewer would expect to work, and precisely the one path where nobody had written a test that varied `now` while holding everything else fixed — every existing test either passed the same `now` as `payment.failed_at` (where the bug is invisible) or checked action/None outcomes that happened to be insensitive to `now` being ignored.

**The fix:** `action_time` inside `decide()`'s scoring loop is now `max(now, payment.failed_at + candidate.offset)`, and the returned offset is `best_action_time - now` instead of the raw candidate offset. On every pre-existing caller `now == payment.failed_at` by construction (harness/`make_full_agent_policy`, `execution/orchestrator.py`) and every candidate offset is non-negative, so `max()` always resolves to the old value — zero behavior change to the primary batch/harness/execution path, verified by the full 245-test suite plus a manual re-run of `make demo`. It only diverges on a genuine re-evaluation, exactly the PROMISE_TO_PAY case. Found and fixed alongside it: `llm.reply_handling.apply_reply_intent`'s `reeval_time` construction (`datetime.combine(promised_date, datetime.min.time())`) dropped timezone info entirely and always landed at midnight — harmless while `now` was ignored, but load-bearing once it wasn't: a naive `reeval_time` compared against an execution/-sourced aware `payment.failed_at` would `TypeError`, and midnight is essentially never inside a customer's declared `contact_hours`, so the fixed `decide()` would have vetoed almost every real promise-to-pay reply on contact hours alone. Fixed to `datetime.combine(promised_date, now.timetz())` — same hour-of-day as `now`, correct tzinfo. Two new tests in `tests/test_full_agent.py` prove `now` is load-bearing (an offset that changes; a contact-hours veto that flips from allowed to vetoed purely from re-evaluating later), and the existing promise-to-pay test in `tests/test_llm_reply_handling.py` now asserts the re-evaluated decision genuinely differs from the original diagnosis, not just that it matches an equivalent direct `decide()` call (which is all it checked before — a test that would have passed against the bug).

**Finding 2: the reply-classification pipeline was unreachable from any live path.** `classify_reply()` and `apply_reply_intent()` were built, adversarially tested (the `TypeError`-on-unhashable-intent fix, the extra-field test, `test_llm_fallback.py`'s full-harness raise-on-every-call proof), and — confirmed by grepping every call site outside `tests/` before touching anything — called from nowhere else in the codebase. No script, route, or scheduler ever fed a real customer reply into them. Concretely: `explicit_opt_out`, which DECISIONS.md's own 2026-08-25 entry calls "the most serious compliance failure available to this system," could not fire in any live run, because nothing durable ever got written for it to act on outside a test's own hand-built `ContactTracker`.

**Why it was dangerous:** the single guardrail this codebase's own writing treats as most safety-critical was, in a real deployment, inert. Not a false entry in the audit log (the fourth-instance-of-the-family shape named above) — worse in a specific way: no entry at all could ever be written, because nothing routed a real reply to the code that would write it.

**The fix:** `execution/reply_webhook.py` (new) — a `POST /replies` route, same shape as `webhook.py`'s existing Razorpay webhook: `handle_reply()` does the actual work with `now` explicit (never read from the clock inside it, same split `webhook.py` already uses between its route and `process_webhook_event`), and the route itself calls `utc_now()` at the request boundary, the one place a live system legitimately reads the real clock. Needed one new query-layer function: `execution/query_layer.py`'s `payment_by_id()`, factored out of `recent_failures()`'s existing reconstruction logic (`_payment_from_diagnosed_event()`, now shared by both) rather than duplicated — a live inbound reply names the one specific payment it's about, not a time window. Verified end to end, through the real route (`fastapi.testclient.TestClient`, in-process ASGI, no real network socket — same "no network calls" guarantee as every other `make` target): a customer replies STOP → `classify_reply` → `apply_reply_intent` → a durable `CustomerOptOutEvent` → a **brand-new engine/session** reconnecting to the same file (not the one that wrote it) → `query_layer.contact_tracker_for()` reconstructs the tracker → `full_agent.decide()` for the customer's next payment is vetoed by `explicit_opt_out`, recorded as `ABANDONED(policy_stop)`. Five new tests in `tests/test_execution_reply_webhook.py`; added as `make reply-demo` (`scripts/demo_reply.py`) so the same chain is reproducible as its own one-command demo, not just provable in a test.

**Known interface gap, reported not hidden, same class as query_layer.py's existing `outage_events` gap:** `apply_reply_intent` needs a full `simulator.types.Customer` persona (`contact_hours`, etc.), and nothing under `execution/` persists or reconstructs one — `Customer` is simulator-side ground truth with no execution/-side analog. The route accepts `contact_hours` as an explicit request field and defaults the rest to inert placeholders (never read by this path), the same "caller pre-binds what execution/ doesn't have a store for" pattern already established for `full_agent`'s `failure_log`/`outage_events`. A real deployment would source this from a merchant's actual customer-profile store, which doesn't exist in this codebase and is out of scope for this round.

**Finding 3 (minor): `outage_detector.py`'s docstring was false.** It said "Deliberately separate from the decision policy and NOT wired into full_agent or the harness yet" — true when the file was first built standalone (2026-08-25's "Systemic outage detector built standalone" entry, above), false by the very next entry the same day ("full_agent gets a real data source"), when `full_agent.py` started importing and calling `detect_systemic_event` directly. The docstring was simply never updated after that — a small, concrete instance of the exact hazard the "recurring pattern" section already names for the audit log, now shown to apply just as easily to a code comment as to a logged event: a claim that was true when written and never re-checked against the code it describes. Swept for others with the same shape (`grep` across `src/` for "not wired"/"not yet"/"never wired"/"unreachable"/etc.) — one more found and fixed in the same pass, in `tests/test_full_agent.py`: a comment claiming a test's `payment` had `failed_at` at midnight, when the test actually uses the file's `DEFAULT_FAILED_AT` (10am) — the test's conclusion was right, the stated reason for it wasn't, which is exactly the "correct behavior, false recorded reason" shape this file already tracks, just in a comment instead of an event log. Fixed both; no other stale claims found in the sweep.

**How this was found:** none of the three by our own testing or our own audit — an external cold review of the repository, reading the code (not the build history) fresh, exactly the "stranger's eyes" this file's own closing section admits our internal process can't fully substitute for. Each was independently verified before any fix — grepped, read, reproduced — rather than taken on the reviewer's word, per instruction and consistent with how every other entry in this log insists on confirming before trusting.

**Lesson:** the "recurring pattern" and "invisible because nothing was complaining" sections above (2026-08-25) already named the general shape of both families these three findings belong to — a false or missing reason in the log, and a component nothing exercises. What this round adds: the same two families apply beyond the append-only event log itself. A docstring is a claim that can go stale exactly like a logged reason can. A dead function parameter is a claim of purity that can be false exactly like a missing audit entry is a claim of safety that isn't backed. And a component with 238 passing tests can still be completely unreachable in production if every one of those tests calls it directly instead of through whatever would actually route to it live — the same "a green suite proves the tests that exist are passing, not that the design got built" lesson from the 8-item audit, recurring a third time, this time surfaced by someone else's eyes instead of our own.

**Video line:** An outside reviewer found in twenty minutes what three rounds of our own audits didn't: a promise-to-pay feature that silently did nothing, and our most important compliance guardrail wired to nowhere at all.
