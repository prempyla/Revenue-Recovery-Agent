...
## The self-audit and what it found unbuilt

**Stage:** After the simulator, the real execution layer (state machine, outbox, webhook receiver, reconciliation poller), and the LLM layer were all independently built and independently tested — before any of them had been checked against each other end to end.

**What broke:** Nothing crashed. This was a deliberate, structured self-review across the whole system — an 8-point audit — run specifically to catch gaps that only show up when you ask "does this actually fit together," not "does this component pass its own tests." Two of the findings were written up and fixed as a single piece of work, since one was meaningless without the other:
- `decide()` (the decision policy) could return "retry in 4 hours," but the execution layer had no concept of "later" at all — every scheduled intent fired the instant a worker ran, regardless of what offset the policy had actually decided on. The offset was being computed and then discarded (the variable was literally named `_offset` to mark it unused).
- `full_agent` — the actual cost-aware, outage-aware decision policy that had been built and evaluated extensively in the simulator — had never once driven the real execution path. Only the simpler `rules_only` policy had ever been wired into `execution/`. All of the real-money-adjacent plumbing had only ever been proven against the less capable policy.

**Why it was dangerous:** Both gaps are the kind that don't show up in any single component's tests, because each component was individually correct — `decide()` correctly computed an offset, the execution layer correctly executed whatever it was given immediately. The defect only exists in the seam between them. A demo built on either gap alone would have looked complete (real Razorpay calls, real webhooks, real state transitions) while quietly running the less sophisticated policy, or running the right policy with none of its timing decisions actually respected.

**How I found it:** A deliberate structured review across the whole system, not a test failure or a live incident — the value here was going looking for integration gaps on purpose rather than waiting for one to surface on its own.

**The fix:** `OutboxIntent` gained an indexed `due_at` column; `run_outbox_worker_once` now filters on `due_at <= now` (with `now` passed in, never read from a clock inside — the same discipline the decision policy itself already followed) instead of dispatching everything unconditionally. `orchestrator.diagnose_and_schedule` gained a `policy_fn` parameter instead of a hardcoded import, and `full_agent` is wired in via a factory that pre-binds its extra batch-level dependencies (failure log, outage events, contact tracker) before handing it in. Commit `0a803b1`. The remaining six audit items were explicitly scoped out of that round rather than silently deferred — the commit message names them as untouched, not resolved.

**Lesson:** Individually well-tested components can still be wrong about each other — schedule a deliberate integration review, don't wait for the seam to fail on its own.

**Video line:** Every piece passed its own tests — the real bug was that our best policy had never actually been wired into the real payment path.
